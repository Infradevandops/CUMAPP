...
class CumAppDemo:
    ...
    async def demo_textverified_integration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            balance_response = await self.session.get(
                f"{self.base_url}/api/account/textverified/balance"
            )
            if balance_response.status_code == 200:
                ...
                print(
                    "⚠️  TextVerified balance check failed (service may not be configured)"
                )

            # Get available services
            services_response = await self.session.get(
                f"{self.base_url}/api/services/textverified"
            )
            if services_response.status_code == 200:
                ...
                print(
                    "⚠️  TextVerified services list failed (service may not be configured)"
                )
    ...
    async def demo_sms_functionality(self) -> Dict[str, Any]:
        ...
        try:
            ...
            sms_response = await self.session.post(
                f"{self.base_url}/api/sms/send", json=sms_data
            )
            ...
            history_response = await self.session.get(
                f"{self.base_url}/api/mock/sms/history?limit=10"
            )
            ...
            if history_response.status_code == 200:
                ...
                for msg in history_data["messages"][-3:]:  # Show last 3 messages
                    print(
                        f"   📱 {msg['timestamp'][:19]} | To: {msg['to']} | {msg['body'][:50]}..."
                    )
    ...
    async def demo_ai_features(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if intent_response.status_code == 200:
                ...
                print(
                    f"   Suggested Tone: {intent_data.get('suggested_tone', 'unknown')}"
                )
            else:
            ...
            conversation = [
                {
                    "role": "user",
                    "content": "Hi, I'm having trouble with WhatsApp verification",
                },
                {
                    "role": "assistant",
    ...
    async def demo_number_management(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for country in countries:
                numbers_response = await self.session.get(
                    f"{self.base_url}/api/numbers/available/{country}"
                )

                if numbers_response.status_code == 200:
                    numbers_data = numbers_response.json()
                    print(
                        f"   {country}: {len(numbers_data['available_numbers'])} numbers available"
                    )
                    ...
                    if numbers_data["available_numbers"]:
                        ...
                        print(
                            f"     Example: {example['phone_number']} - {example['monthly_cost']}/month"
                        )
    ...
    async def demo_platform_statistics(self) -> Dict[str, Any]:
        ...
        try:
            stats_response = await self.session.get(
                f"{self.base_url}/api/mock/statistics"
            )
