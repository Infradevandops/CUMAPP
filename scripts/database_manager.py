    ...
    def get_migration_status(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "history": history,
                "status": "up_to_date" if current_revision != "unknown" else "needs_migration"
            }
    ...
    def _generate_performance_recommendations(self, analysis: Dict) -> List[str]:
        ...
        for table_name, size_info in analysis["table_sizes"].items():
            if size_info["size_bytes"] > 100 * 1024 * 1024:  # > 100MB
                recommendations.append(f"Large table '{table_name}' ({size_info['size']}) - consider partitioning or archiving old data")
        
        # Check for unused indexes
        for table_name, indexes in analysis["index_usage"].items():
            for index in indexes:
                if index["reads"] == 0:
                    recommendations.append(f"Unused index '{index['index_name']}' on table '{table_name}' - consider dropping")
        
        return recommendations
    
    def optimize_database(self, vacuum_analyze: bool = True, reindex: bool = False) -> Dict:
        """
    ...
    def backup_database(self, backup_path: str, compress: bool = True) -> bool:
        ...
        try:
            ...
            cmd = [
                ...
                "-d", parsed.path.lstrip('/') if parsed.path else "cumapp",
                "-f", backup_path
            ]
        ...
        try:
            ...
            with self.SessionLocal() as db:
                ...
                if health_status["table_count"] == 0:
                    health_status["issues"].append("No tables found - database may not be initialized")
...
async def main():
    ...
    parser.add_argument('command', choices=[
        'migrate', 'create-migration', 'status', 'analyze', 'optimize', 
    ...
    parser.add_argument('--vacuum', action='store_true', help='Run VACUUM ANALYZE (for optimize)')
    parser.add_argument('--reindex', action='store_true', help='Reindex tables (for optimize)')
    parser.add_argument('--drop-existing', action='store_true', help='Drop existing database (for restore)')
