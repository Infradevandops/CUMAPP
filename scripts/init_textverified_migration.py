...
def create_system_folders():
    ...
    try:
        ...
        existing_count = db.query(InboxFolder).filter(InboxFolder.is_system_folder == True).count()
        if existing_count > 0:
            print(f"  System folders already exist ({existing_count} folders). Skipping creation.")
            return
...
def verify_database():
    ...
    try:
        ...
        sample_country = db.query(CountryRouting).filter(CountryRouting.country_code == 'US').first()
        if not sample_country:
            raise Exception("Could not find US country routing data")
        
        print(f"  Sample country: {sample_country.country_name} ({sample_country.country_code})")
