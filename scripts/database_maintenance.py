...
class DatabaseMaintenance:
    ...
    def run_data_cleanup(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"✅ Cleanup completed: {total_deleted} records deleted in {total_time}ms"
            )
            ...
            for row in cleanup_results:
                if row.records_deleted > 0:
                    logger.info(
                        f"  📋 {row.table_name}: {row.records_deleted} records deleted"
                    )
    ...
    def check_index_usage(self) -> Dict[str, Any]:
        ...
        try:
            ...
            low_usage_indexes = [
                idx for idx in index_stats if idx.usage_level == "LOW_USAGE"
            ]
            ...
            return {
                ...
                "recommendations": self._generate_index_recommendations(
                    unused_indexes, low_usage_indexes
                ),
            }
    ...
    def _generate_index_recommendations(
        self, unused_indexes: List, low_usage_indexes: List
    ) -> List[str]:
        ...
        if low_usage_indexes:
            recommendations.append(
                f"Review {len(low_usage_indexes)} low-usage indexes for potential optimization"
            )
    ...
    def vacuum_analyze_tables(self, tables: List[str] = None) -> Dict[str, Any]:
        ...
        return {"success": True, "table_results": results, "total_tables": len(tables)}
            ...
            for check_name, query in orphaned_checks:
                ...
                if count > 0:
                    health_status["warnings"].append(
                        f"{check_name}: {count} orphaned records"
                    )
                    orphaned_found = True
    ...
    with DatabaseMaintenance() as maintenance:
        ...
        if stats["success"] and stats["statistics"]:
            ...
            for stat in stats["statistics"]:
                print(
                    f"  📋 {stat['table_name']}: {stat['total_records_deleted']} deleted, "
