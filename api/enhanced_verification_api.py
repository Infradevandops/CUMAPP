...
class VerificationCreateRequest(BaseModel):
    ...
    service_name: str = Field(..., description="Service to verify (e.g., whatsapp, google)")
    capability: str = Field("sms", description="Verification capability (sms, voice)")
    country_preference: Optional[str] = Field(None, description="Preferred country code for number")
    use_smart_routing: bool = Field(True, description="Enable smart routing for optimal delivery")
    priority: str = Field("normal", description="Verification priority (low, normal, high)")
...
class VerificationBulkRequest(BaseModel):
    ...
    country_preference: Optional[str] = Field(None, description="Preferred country code")
    use_smart_routing: bool = Field(True, description="Enable smart routing")
    stagger_delay: int = Field(5, ge=0, le=60, description="Delay between verifications in seconds")

class VerificationRetryRequest(BaseModel):
    """Request model for retrying verification"""
    use_different_number: bool = Field(False, description="Use a different phone number")
    country_preference: Optional[str] = Field(None, description="New country preference")
...
def get_verification_service(db: Session = Depends(get_db)) -> IntegratedVerificationService:
...
async def create_verification(
    ...
    verification_service: IntegratedVerificationService = Depends(get_verification_service)
):
    """
    ...
    try:
        ...
        verification = await verification_service.create_verification_request(
        ...
        return await _convert_to_verification_response(verification, verification_service)
...
async def list_verifications(
    ...
    try:
        ...
        # Get verifications
        result = await verification_service.get_user_verifications(
            user_id=current_user.id,
            page=page,
            page_size=page_size,
            filters=filters
        ...
        for verification in result["verifications"]:
            response = await _convert_to_verification_response(verification, verification_service)
...
async def create_bulk_verifications(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to create bulk verifications")
...
async def get_supported_services(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to retrieve supported services")
...
async def _convert_to_verification_response(
    verification: Any,
    verification_service: IntegratedVerificationService
    ...
    try:
        ...
        service_info = await verification_service.get_service_info(verification.service_name)
        ...
        return VerificationResponse(
            ...
            service_display_name=service_info.get("display_name", verification.service_name.title()),
