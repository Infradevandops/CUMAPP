...
from services.smart_routing_engine import SmartRoutingEngine, create_smart_routing_engine
...
class RoutingRequest(BaseModel):
    ...
    user_numbers: Optional[List[str]] = Field(default=[], description="User's existing numbers")
    message_count: int = Field(default=1, ge=1, description="Expected number of SMS messages")
    call_minutes: int = Field(default=0, ge=0, description="Expected call duration in minutes")
...
class AnalyticsRequest(BaseModel):
    ...
    recent_destinations: List[str] = Field(..., description="Recent destination numbers")
...
def get_routing_engine() -> SmartRoutingEngine:
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail="Smart routing service not available")
...
async def get_routing_recommendation(
    ...
    try:
        ...
        response = RoutingRecommendationResponse(
            ...
            alternative_options=[convert_option(opt) for opt in recommendation.alternative_options],
...
async def calculate_distance(
    ...
    try:
        ...
        delivery_score = routing_engine.calculate_delivery_score(request.country1, request.country2)
...
async def detect_country(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to detect country: {str(e)}")

@router.get("/closest/{country_code}")
async def get_closest_countries(
    country_code: str = Path(..., description="Country code to find closest countries for"),
    limit: int = Query(5, ge=1, le=20, description="Maximum number of countries to return"),
    current_user: User = Depends(get_current_user),
    routing_engine: SmartRoutingEngine = Depends(get_routing_engine)
):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get closest countries: {str(e)}")
