...
from services.integrated_verification_service import IntegratedVerificationService, create_integrated_verification_service
from textverified_client import TextVerifiedClient
from enhanced_twilio_client import EnhancedTwilioClient

# Initialize router and security
router = APIRouter(prefix="/api/integrated-verification", tags=["integrated-verification"])
security = HTTPBearer()
...
class CreateVerificationRequest(BaseModel):
    ...
    preferred_country: Optional[str] = Field(None, description="Preferred country for routing")
...
def get_integrated_verification_service(db: Session = Depends(get_db)) -> IntegratedVerificationService:
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail="Integrated verification service not available")

@router.post("/create", response_model=VerificationWorkflowResponse)
async def create_service_verification(
    request: CreateVerificationRequest,
    current_user: User = Depends(get_current_user),
    verification_service: IntegratedVerificationService = Depends(get_integrated_verification_service)
):
    """
...
async def get_supported_services(
    ...
    """
    try:
        services_info = await verification_service.get_supported_services(category=category)
        
        return SupportedServicesResponse(**services_info)
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to get supported services: {str(e)}")
...
async def get_service_categories(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get service categories: {str(e)}")
...
async def get_verification_analytics_summary(
    ...
    try:
        ...
        filtered_verifications = [
            v for v in history['verifications']
            if datetime.fromisoformat(v['created_at'].replace('Z', '+00:00')) >= cutoff_date
        ]
    ...
    try:
        ...
        return {
            ...
            "components": {
                ...
                "textverified_client": verification_service.textverified_client is not None,
...
async def get_service_info(
    ...
    try:
        ...
        if service_name not in services_info['services']:
            raise HTTPException(status_code=404, detail=f"Service '{service_name}' not supported")
        ...
        detailed_info = {
            **service_info,
            ...
            "supported": True,
            "estimated_cost": "Standard rate" if service_info['cost_tier'] == 'standard' else "Premium rate"
        }
        
        return {
            "success": True,
            "service_info": detailed_info
        }
...
async def create_batch_verifications(
    ...
    preferred_country: Optional[str] = Query(None, description="Preferred country for routing"),
    current_user: User = Depends(get_current_user),
    verification_service: IntegratedVerificationService = Depends(get_integrated_verification_service)
):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create batch verifications: {str(e)}")