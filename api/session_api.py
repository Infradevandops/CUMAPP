...
async def list_user_sessions(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        for session in sessions:
            is_current = (
                current_token
                and session.refresh_token == current_token
...
@router.post("/cleanup")
async def cleanup_expired_sessions(request: Request, db: Session = Depends(get_db)):
    """
...
@router.get("/stats")
async def get_session_stats(request: Request, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        # Count sessions
        total_sessions = (
            db.query(UserSession).filter(UserSession.user_id == current_user.id).count()
        )
        ...
        return {
            ...
            "latest_activity": (
                latest_session.last_used.isoformat() if latest_session else None
            ),
