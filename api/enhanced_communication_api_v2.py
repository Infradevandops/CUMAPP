...
from services.communication_service import CommunicationService, create_communication_service
...
class SendSMSRequest(BaseModel):
    ...
    country_preference: Optional[str] = Field(None, description="Preferred country code for routing")
    priority: str = Field("normal", description="Message priority (low, normal, high)")
    schedule_time: Optional[datetime] = Field(None, description="Schedule message for future delivery")
...
class MakeCallRequest(BaseModel):
    ...
    use_smart_routing: bool = Field(True, description="Enable smart routing for optimal routing")
    country_preference: Optional[str] = Field(None, description="Preferred country code for routing")
    twiml_url: Optional[str] = Field(None, description="TwiML URL for call instructions")
...
class CallControlRequest(BaseModel):
    ...
    action: str = Field(..., description="Action to perform (record, stop_record, forward, hangup)")
...
class NumberManagementRequest(BaseModel):
    ...
    country_code: Optional[str] = Field(None, description="Country code for number search/purchase")
    area_code: Optional[str] = Field(None, description="Area code preference")
    number_type: str = Field("local", description="Number type (local, toll_free, mobile)")
    capabilities: List[str] = Field(["sms", "voice"], description="Required capabilities")
...
class ConversationFilterRequest(BaseModel):
    ...
    message_type: Optional[str] = Field(None, description="Filter by message type (sms, mms, voice)")
...
async def receive_voice_webhook(
    ...
    try:
        ...
        return Response(content="<?xml version='1.0' encoding='UTF-8'?><Response><Say>Sorry, we're experiencing technical difficulties.</Say></Response>", 
                       media_type="application/xml")
