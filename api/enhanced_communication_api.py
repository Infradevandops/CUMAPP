...
router = APIRouter(prefix="/api/enhanced-communication", tags=["enhanced-communication"])
security = HTTPBearer()
...
def get_enhanced_twilio_client() -> EnhancedTwilioClient:
    ...
    if not enhanced_twilio_client:
        raise HTTPException(status_code=503, detail="Enhanced Twilio service not available")
...
async def receive_call_webhook(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to process call webhook: {str(e)}")
...
async def record_call(
    ...
    try:
        result = await twilio_client.record_call(
            ...
            record_from_start=request.record_from_start
        )
...
async def create_conference(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create conference: {str(e)}")
...
async def list_owned_numbers(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to list owned numbers: {str(e)}")
...
async def get_number_info(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get number info: {str(e)}")
