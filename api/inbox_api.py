...
class InboxMessage(BaseModel):
    ...
    id: str
    type: str = Field(..., description="Message type: verification_code, conversation, notification")
    ...
    verification_id: Optional[str] = Field(None, description="Associated verification ID")
...
async def get_inbox_messages(
    ...
    try:
        ...
        stats = InboxStats(
            ...
            verification_codes=len([m for m in mock_messages if m.type == "verification_code"]),
...
async def mark_all_messages_as_read(
    current_user: User = Depends(get_current_user_from_middleware),
    message_type: Optional[str] = Query(None, description="Mark only specific type as read")
):
    """
...
async def get_inbox_stats(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to retrieve inbox statistics")