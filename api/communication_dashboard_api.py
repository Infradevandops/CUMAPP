...
class Conversation(BaseModel):
    ...
    from_number: str = Field(
        ..., description="User's phone number used for this conversation"
    )
...
class AISuggestionResponse(BaseModel):
    ...
    context_used: bool = Field(
        True, description="Whether conversation context was used"
    )
...
async def get_ai_suggestions(
    request: AISuggestionRequest, current_user: User = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        return AISuggestionResponse(
            suggestions=contextual_suggestions, context_used=True
        )
...
async def get_communication_stats(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve communication statistics"
        )
...
async def mark_conversation_as_read(
    ...
    try:
        ...
        logger.info(
            f"Marking conversation {conversation_id} as read for user {current_user.id}"
        )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to mark conversation as read"
        )
...
async def delete_conversation(
    ...
    try:
        ...
        logger.info(
            f"Deleting conversation {conversation_id} for user {current_user.id}"
        )
