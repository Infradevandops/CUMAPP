...
class NumberPurchaseRequest(BaseModel):
    ...
    usage_plan: str = Field("standard", description="Usage plan (basic, standard, premium)")
    webhook_url: Optional[str] = Field(None, description="Webhook URL for incoming messages/calls")
...
class NumberOptimizationRequest(BaseModel):
    ...
    target_countries: List[str] = Field(..., description="Target countries for optimization")
    monthly_message_volume: int = Field(100, ge=1, description="Expected monthly message volume")
    monthly_call_minutes: int = Field(60, ge=0, description="Expected monthly call minutes")
    budget_limit: Optional[float] = Field(None, description="Monthly budget limit")
    optimization_goal: str = Field("cost", description="Optimization goal (cost, delivery, coverage)")
...
def get_routing_engine(twilio_client: EnhancedTwilioClient = Depends(get_twilio_client)) -> SmartRoutingEngine:
...
async def search_available_numbers(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to search available numbers")
...
async def get_number_analytics(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to retrieve number analytics")
...
async def setup_purchased_number(phone_number: str, user_id: str, webhook_url: Optional[str]):
    """
