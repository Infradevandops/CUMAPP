...
class ResponseSuggestionsRequest(BaseModel):
    ...
    num_suggestions: int = Field(3, ge=1, le=10, description="Number of suggestions to generate")
...
class ConversationEnhancementRequest(BaseModel):
    ...
    conversation_id: str = Field(..., description="Conversation identifier")
    enhancement_type: str = Field("auto", description="Type of enhancement (auto, summary, sentiment, topics, actions)")
...
async def create_conversation(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to create conversation context")

@router.post("/conversations/{conversation_id}/messages", response_model=ConversationContextResponse)
...
async def generate_response_suggestions(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to generate response suggestions")
...
async def health_check(
    ...
    """
    try:
        model_loaded = hasattr(ai_service, 'local_model') and ai_service.local_model is not None
...
async def list_conversations(
    ...
    try:
        ...
        for context in list(ai_service.conversation_contexts.values())[:limit]:
            conversations.append(ConversationContextResponse(
                conversation_id=context.conversation_id,
                participants=context.participants,
                message_count=len(context.messages),
                created_at=context.created_at,
...
async def log_suggestion_usage(conversation_id: str, suggestion_count: int):
    ...
    """
    try:
        logger.info(f"AI suggestions generated - Conversation: {conversation_id}, Count: {suggestion_count}")
