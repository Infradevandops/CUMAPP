...
class SendSMSRequest(BaseModel):
    ...
    use_smart_routing: bool = Field(True, description="Use smart routing for optimal number selection")
...
async def receive_sms_webhook(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to process SMS webhook: {str(e)}")
...
async def make_voice_call_with_routing(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to make voice call: {str(e)}")
...
async def receive_voice_call_webhook(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to process voice call webhook: {str(e)}")
...
async def record_call(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to start call recording: {str(e)}")
...
async def create_conference_call(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create conference call: {str(e)}")

# History Management Endpoints
@router.get("/history/conversations")
async def get_conversation_history(
    conversation_id: Optional[str] = Query(None, description="Specific conversation ID"),
    external_number: Optional[str] = Query(None, description="External number to filter by"),
...
async def manage_user_number(
    ...
    try:
        if request.action not in ["purchase", "release", "update"]:
            raise HTTPException(status_code=400, detail="Invalid action. Must be: purchase, release, or update")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to manage number: {str(e)}")
