...
from middleware.auth_middleware import get_current_user_from_middleware as get_current_user
...
class CostComparison(BaseModel):
    ...
    monthly_savings: float = Field(..., description="Estimated monthly savings for 100 SMS")
...
async def get_supported_countries(
    ...
    try:
        # Mock country data
        countries = [
            Country(code="US", name="United States", flag="🇺🇸", dial_code="+1", base_cost=0.01, popular=True),
            Country(code="GB", name="United Kingdom", flag="🇬🇧", dial_code="+44", base_cost=0.02, popular=True),
            Country(code="CA", name="Canada", flag="🇨🇦", dial_code="+1", base_cost=0.01, popular=True),
            Country(code="AU", name="Australia", flag="🇦🇺", dial_code="+61", base_cost=0.03, popular=True),
            Country(code="DE", name="Germany", flag="🇩🇪", dial_code="+49", base_cost=0.02, popular=True),
            Country(code="FR", name="France", flag="🇫🇷", dial_code="+33", base_cost=0.02, popular=True),
            Country(code="JP", name="Japan", flag="🇯🇵", dial_code="+81", base_cost=0.04, popular=False),
            Country(code="BR", name="Brazil", flag="🇧🇷", dial_code="+55", base_cost=0.03, popular=False),
            Country(code="IN", name="India", flag="🇮🇳", dial_code="+91", base_cost=0.02, popular=True),
            Country(code="CN", name="China", flag="🇨🇳", dial_code="+86", base_cost=0.05, popular=False),
            Country(code="MX", name="Mexico", flag="🇲🇽", dial_code="+52", base_cost=0.02, popular=False),
            Country(code="ES", name="Spain", flag="🇪🇸", dial_code="+34", base_cost=0.02, popular=False),
            Country(code="IT", name="Italy", flag="🇮🇹", dial_code="+39", base_cost=0.025, popular=False),
            Country(code="NL", name="Netherlands", flag="🇳🇱", dial_code="+31", base_cost=0.02, popular=False),
            Country(code="SE", name="Sweden", flag="🇸🇪", dial_code="+46", base_cost=0.025, popular=False)
        ]
...
async def get_routing_recommendations(
    country_code: str,
    current_user: User = Depends(get_current_user),
    message_count: int = Query(1, ge=1, le=1000, description="Number of messages to send")
):
    """
    ...
    try:
        ...
        routing_options = [
            ...
            RoutingOption(
                ...
                savings_percentage=int(((direct_cost - regional_cost) / direct_cost) * 100)
            )
        ]
        ...
        raise HTTPException(status_code=500, detail="Failed to get routing recommendations")
...
async def purchase_number_and_send(
    ...
    try:
        ...
        recommendations = await get_routing_recommendations(request.country_code, current_user)
        selected_route = next((r for r in [recommendations.recommended_route] + recommendations.alternative_routes 
                              if r.id == request.route_id), None)
        
        if not selected_route:
            raise HTTPException(status_code=404, detail="Route not found")
        
        # Calculate costs
        number_cost = 2.00 if selected_route.type == "local" else 0.00  # $2 for local number
        ...
        raise HTTPException(status_code=500, detail="Failed to purchase number and send SMS")
...
async def get_cost_estimate(
    ...
    """
    try:
        recommendations = await get_routing_recommendations(country_code, current_user, message_count)
        
        estimates = []
        for route in [recommendations.recommended_route] + recommendations.alternative_routes:
