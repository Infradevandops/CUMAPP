...
from services.subscription_service import SubscriptionService, create_subscription_service
...
class PurchaseSubscriptionRequest(BaseModel):
    ...
    billing_cycle: str = Field(default="monthly", description="Billing cycle: monthly, quarterly, yearly")
    payment_method: Optional[Dict[str, Any]] = Field(None, description="Payment method information")

class TrackUsageRequest(BaseModel):
    usage_type: str = Field(..., description="Type of usage: sms_sent, voice_minutes, verifications")
    amount: int = Field(default=1, ge=1, description="Amount of usage to track")
    metadata: Optional[Dict[str, Any]] = Field(None, description="Additional metadata")

class CancelSubscriptionRequest(BaseModel):
    immediate: bool = Field(default=False, description="Cancel immediately or at end of billing period")
...
async def get_subscription_plans(
    ...
    """
    try:
        plans = await subscription_service.get_subscription_plans(include_pricing=include_pricing)
...
async def get_current_billing_info(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get current billing info: {str(e)}")
...
async def compare_subscription_plans(
    ...
    """
    try:
        all_plans = await subscription_service.get_subscription_plans(include_pricing=True)
        ...
        for feature in features_set:
            ...
            for plan_id in plan_ids:
                ...
                comparison["comparison_matrix"][feature][plan_id] = feature in plan_features
...
async def get_plan_recommendations(
    ...
    try:
        ...
        if any(percentage > 80 for percentage in utilization.values()):
            for plan_id, plan_info in all_plans["plans"].items():
                if plan_id != current_plan:
                    ...
                    if (new_limits["sms_monthly"] > current_limits["sms_monthly"] or 
                        new_limits["sms_monthly"] == -1):
        ...
        elif all(percentage < 30 for percentage in utilization.values()) and current_plan != "free":
        ...
        if not recommendations:
            recommendations.append({
                ...
                "monthly_price": all_plans["plans"][current_plan]["pricing"]["monthly"]
            })
