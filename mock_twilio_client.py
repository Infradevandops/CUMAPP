...
class MockTwilioClient:
    ...
    def get_usage_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_cost": len(self.message_history) * 0.0075
            + len(self.call_history) * 0.02,
            "current_month": {
                "messages": len(
                    [
                        m
                        for m in self.message_history
                        if datetime.fromisoformat(m["timestamp"]).month
                        == datetime.now().month
                    ]
                ),
                "calls": len(
...
def create_twilio_client(
    account_sid: str = None, auth_token: str = None, use_mock: bool = True
):
    """
...
if __name__ == "__main__":
    ...
    try:
        message = mock_client.messages.create(
            body="Hello from Mock Twilio!", from_="+1555000001", to="+1234567890"
        )
