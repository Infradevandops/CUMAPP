...
from models import (Base, Conversation, ConversationStatus, Message,
                    MessageType, SubscriptionPlan, User, UserRole,
...
class TestConversationModel:
    ...
    def test_create_conversation_basic(self, db_session, test_users):
        ...
        conversation = Conversation(
            title="Test Conversation", created_by=creator.id, is_group=False
        )
    ...
    def test_create_group_conversation(self, db_session, test_users):
        ...
        conversation = Conversation(
            title="Group Chat", created_by=creator.id, is_group=True
        )
    ...
    def test_conversation_status_enum(self, db_session, test_users):
        ...
        for status in ConversationStatus:
            conversation = Conversation(
                title=f"Test {status.value}", created_by=creator.id, status=status
            )
    ...
    def test_conversation_archive_and_mute(self, db_session, test_users):
        ...
        conversation = Conversation(title="Test Settings", created_by=creator.id)
    ...
    def test_conversation_relationships(self, db_session, test_users):
        ...
        conversation = Conversation(title="Relationship Test", created_by=creator.id)
...
class TestMessageModel:
    ...
    def test_create_chat_message(self, db_session, test_users):
        ...
        conversation = Conversation(title="Test Chat", created_by=creator.id)
        db_session.add(conversation)
        db_session.commit()
        db_session.refresh(conversation)
    ...
    def test_create_sms_message(self, db_session, test_users):
        ...
        conversation = Conversation(
            title="SMS Test", created_by=creator.id, external_number="+1234567890"
        )
        db_session.add(conversation)
        db_session.commit()
        db_session.refresh(conversation)
    ...
    def test_message_delivery_tracking(self, db_session, test_users):
        ...
        conversation = Conversation(title="Delivery Test", created_by=creator.id)
        db_session.add(conversation)
        db_session.commit()
        db_session.refresh(conversation)
    ...
    def test_message_editing_and_deletion(self, db_session, test_users):
        ...
        conversation = Conversation(title="Edit Test", created_by=creator.id)
        db_session.add(conversation)
        db_session.commit()
        db_session.refresh(conversation)
    ...
    def test_message_type_enum(self, db_session, test_users):
        ...
        conversation = Conversation(title="Type Test", created_by=creator.id)
        db_session.add(conversation)
        db_session.commit()
        db_session.refresh(conversation)
...
class TestConversationParticipants:
    ...
    def test_add_participants(self, db_session, test_users):
        ...
        conversation = Conversation(
            title="Participants Test", created_by=creator.id, is_group=True
        )
    ...
    def test_remove_participants(self, db_session, test_users):
        ...
        conversation = Conversation(
            title="Remove Test", created_by=creator.id, is_group=True
        )
    ...
    def test_user_conversations_relationship(self, db_session, test_users):
        ...
        conversations = []
        for i in range(3):
            conv = Conversation(title=f"Test Conversation {i}", created_by=creator.id)
...
class TestModelValidation:
    ...
    def test_message_required_fields(self, db_session, test_users):
        ...
        conversation = Conversation(title="Test", created_by=creator.id)
        db_session.add(conversation)
        db_session.commit()
        db_session.refresh(conversation)
