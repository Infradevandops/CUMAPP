...
class TestEnhancedSearch:
    ...
    @pytest.mark.asyncio
    async def test_enhanced_message_search_basic(
        self, mock_db, mock_user, mock_messages
    ):
        """Test basic message search functionality"""
        service = ConversationService(mock_db)

        # Mock database queries
        mock_db.query.return_value.join.return_value.filter.return_value.subquery.return_value = (
            Mock()
        )
        mock_db.query.return_value.options.return_value.filter.return_value.filter.return_value.count.return_value = len(
            mock_messages
        )
        mock_db.query.return_value.options.return_value.filter.return_value.filter.return_value.order_by.return_value.offset.return_value.limit.return_value.all.return_value = (
            mock_messages
        )

        # Test search
        result_messages, total_count = await service.search_messages(
            user_id=mock_user.id, query="search terms", limit=10, offset=0
        )
    ...
    @pytest.mark.asyncio
    async def test_enhanced_message_search_with_filters(
        self, mock_db, mock_user, mock_messages
    ):
        ...
        mock_db.query.return_value.options.return_value.filter.return_value.filter.return_value.filter.return_value.count.return_value = (
            5
        )
    ...
    async def test_conversation_search(self, mock_db, mock_user):
        ...
        # Mock database queries
        mock_db.query.return_value.options.return_value.join.return_value.filter.return_value.filter.return_value.count.return_value = len(
            mock_conversations
        )
        mock_db.query.return_value.options.return_value.join.return_value.filter.return_value.filter.return_value.order_by.return_value.offset.return_value.limit.return_value.all.return_value = (
            mock_conversations
        )

        result_conversations, total_count = await service.search_conversations(
            user_id=mock_user.id, query="Test", limit=10, offset=0
        )
    ...
    async def test_user_mentions_search(self, mock_db, mock_user):
        ...
        result_users = await service.get_user_mentions(
            user_id=mock_user.id, query="test", limit=10
        )
    ...
    @pytest.mark.asyncio
    async def test_user_mentions_with_conversation_context(
...
class TestInfiniteScroll:
    ...
    async def test_get_conversation_messages_with_cursor(self, mock_db, mock_user):
        ...
        before_message = Message(
            id="msg-25", created_at=base_time - timedelta(minutes=25)
        )
        ...
        with patch.object(service, "get_conversation", return_value=mock_conversation):
            ...
            mock_db.query.return_value.options.return_value.filter.return_value.filter.return_value.filter.return_value.count.return_value = len(
                mock_messages
            )
            mock_db.query.return_value.options.return_value.filter.return_value.filter.return_value.filter.return_value.order_by.return_value.offset.return_value.limit.return_value.all.return_value = (
                mock_messages
            )
...
class TestSearchAPI:
    ...
    def test_search_messages_endpoint(self, client):
        ...
        with patch("api.conversation_api.get_current_active_user") as mock_auth:
            ...
            with patch(
                "api.conversation_api.ConversationService"
...
class TestEnhancedChatInterface:
    ...
    def test_enhanced_chat_with_search_endpoint(self, client):
        ...
        with patch("api.enhanced_chat_api.get_current_active_user") as mock_auth:
            ...
            with patch(
                "api.enhanced_chat_api.ConversationService"
            ) as mock_service_class:
                mock_service = Mock()
                mock_service_class.return_value = mock_service
