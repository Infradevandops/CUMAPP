...
from api.enhanced_verification_api import router
...
class TestEnhancedVerificationAPI:
    ...
    def test_create_verification_success(self, mock_verification_service, mock_user, sample_verification):
        """Test successful verification creation"""
        mock_verification_service.create_verification_request.return_value = sample_verification
        ...
        with patch('api.enhanced_verification_api.get_current_user', return_value=mock_user), \
             patch('api.enhanced_verification_api.get_verification_service', return_value=mock_verification_service):
            
            response = client.post("/api/verification/create", json={
    ...
    def test_list_verifications_success(self, mock_verification_service, mock_user, sample_verification):
    ...
    def test_list_verifications_with_filters(self, mock_verification_service, mock_user, sample_verification):
        ...
        with patch('api.enhanced_verification_api.get_current_user', return_value=mock_user), \
            ...
            response = client.get("/api/verification?service_name=whatsapp&status=pending&country_code=US")
    ...
    def test_get_verification_success(self, mock_verification_service, mock_user, sample_verification):
        """Test successful verification retrieval"""
        mock_verification_service.get_verification_details.return_value = sample_verification
        mock_verification_service.get_service_info.return_value = {
            "display_name": "WhatsApp",
            "average_delivery_time": 30,
            "success_rate": 0.95
    ...
    def test_get_verification_not_found(self, mock_verification_service, mock_user):
        ...
        mock_verification_service.get_verification_details.side_effect = ValueError("Verification not found")
    ...
    def test_get_verification_messages_success(self, mock_verification_service, mock_user):
        """Test successful verification messages retrieval"""
        mock_verification_service.get_verification_messages.return_value = {
            "messages": [
                {"content": "Your WhatsApp code is 123456", "timestamp": datetime.utcnow()},
    ...
    def test_get_verification_messages_with_force_refresh(self, mock_verification_service, mock_user):
        ...
        with patch('api.enhanced_verification_api.get_current_user', return_value=mock_user), \
            ...
            response = client.get("/api/verification/ver_123/messages?force_refresh=true")
    ...
    def test_retry_verification_success(self, mock_verification_service, mock_user, sample_verification):
        ...
        with patch('api.enhanced_verification_api.get_current_user', return_value=mock_user), \
            ...
            response = client.post("/api/verification/ver_123/retry", json={
                "use_different_number": True,
    ...
    def test_retry_verification_invalid_request(self, mock_verification_service, mock_user):
        """Test verification retry with invalid request"""
        mock_verification_service.retry_verification.side_effect = ValueError("Cannot retry completed verification")
        
        with patch('api.enhanced_verification_api.get_current_user', return_value=mock_user), \
             patch('api.enhanced_verification_api.get_verification_service', return_value=mock_verification_service):
            
            response = client.post("/api/verification/ver_123/retry", json={
                "use_different_number": False
            })
    ...
    def test_create_bulk_verifications_success(self, mock_verification_service, mock_user, sample_verification):
        ...
        mock_verification_service.create_bulk_verifications.return_value = [verification1, verification2]
        mock_verification_service.get_service_info.side_effect = [
            {"display_name": "WhatsApp", "average_delivery_time": 30, "success_rate": 0.95},
            {"display_name": "Google", "average_delivery_time": 15, "success_rate": 0.98}
        ]
        
        with patch('api.enhanced_verification_api.get_current_user', return_value=mock_user), \
             patch('api.enhanced_verification_api.get_verification_service', return_value=mock_verification_service):
            
            response = client.post("/api/verification/bulk", json={
    ...
    def test_get_supported_services_success(self, mock_verification_service):
        ...
        with patch('api.enhanced_verification_api.get_verification_service', return_value=mock_verification_service):
    ...
    def test_get_supported_services_with_category_filter(self, mock_verification_service):
        ...
        with patch('api.enhanced_verification_api.get_verification_service', return_value=mock_verification_service):
            response = client.get("/api/verification/services/supported?category=messaging")
        ...
        mock_verification_service.get_supported_services.assert_called_with(category="messaging")
    
    def test_get_verification_statistics_success(self, mock_verification_service, mock_user):
    ...
    def test_get_verification_statistics_with_service_filter(self, mock_verification_service, mock_user):
        ...
        with patch('api.enhanced_verification_api.get_current_user', return_value=mock_user), \
            ...
            response = client.get("/api/verification/stats/summary?period_days=30&service_name=whatsapp")
    ...
    def test_export_verification_data_with_filters(self, mock_verification_service, mock_user):
        ...
        with patch('api.enhanced_verification_api.get_current_user', return_value=mock_user), \
            ...
            response = client.get("/api/verification/export/data?service_name=whatsapp&status=completed")
    ...
    def test_health_check_error(self, mock_verification_service):
        ...
        mock_verification_service.check_service_health.side_effect = Exception("Service unavailable")
    ...
    def test_invalid_verification_id_format(self, mock_verification_service, mock_user):
        ...
        mock_verification_service.get_verification_details.side_effect = ValueError("Invalid verification ID format")
...
class TestEnhancedVerificationAPIIntegration:
    ...
    def test_full_verification_workflow(self):
        ...
        for expected_route in expected_routes:
            ...
            route_exists = any(expected_route.replace("{verification_id}", "test") in route 
