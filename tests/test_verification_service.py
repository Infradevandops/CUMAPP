...
def verification_service(db_session):
    """Create verification service with real dependencies"""
    textverified_client = TextVerifiedClient()
    return VerificationService(db_session, textverified_client)
...
class TestCodeExtractionService:
    ...
    def test_extract_service_specific_codes(self):
        ...
        for message, service, expected in test_cases:
            ...
            assert (
                codes == expected
            ), f"Failed for service {service}, message: {message}"
...
class TestVerificationService:
    ...
    async def test_create_verification_success(
        self, verification_service, test_user, mock_textverified_client
    ):
        """Test successful verification creation"""
        result = await verification_service.create_verification(
            user_id=test_user.id, service_name="whatsapp", capability="sms"
        )
    ...
    async def test_create_verification_invalid_user(self, verification_service):
        ...
        with pytest.raises(ValueError, match="User .* not found"):
            await verification_service.create_verification(
                user_id="invalid-user", service_name="whatsapp"
            )
    ...
    async def test_create_verification_no_client(self, db_session, test_user):
        ...
        with pytest.raises(ValueError, match="TextVerified client not configured"):
            await service.create_verification(
                user_id=test_user.id, service_name="whatsapp"
            )

    @pytest.mark.asyncio
    async def test_get_verification_number(
        self, verification_service, test_user, mock_textverified_client
    ):
        ...
        phone_number = await verification_service.get_verification_number(
            user_id=test_user.id, verification_id=verification.id
        )
    ...
    @pytest.mark.asyncio
    async def test_check_verification_messages_with_codes(
        self, verification_service, test_user, mock_textverified_client
    ):
    ...
    async def test_get_verification_history_with_filters(
        self, verification_service, test_user
    ):
        ...
        completed_history = await verification_service.get_verification_history(
            test_user.id, filters={"status": "completed"}
        )
        ...
        whatsapp_history = await verification_service.get_verification_history(
            test_user.id, filters={"service_name": "whatsapp"}
        )
    ...
    async def test_search_verifications(self, verification_service, test_user):
        ...
        results = await verification_service.search_verifications(
            test_user.id, "whatsapp"
        )
    ...
    async def test_cancel_verification(
        ...
        assert result == True
        mock_textverified_client.cancel_verification.assert_called_once_with(
            "tv_123456"
        )
    ...
    async def test_export_verification_data_json(self, verification_service, test_user):
        ...
        export_result = await verification_service.export_verification_data(
            user_id=test_user.id, format_type="json"
        )
    ...
    async def test_export_verification_data_csv(self, verification_service, test_user):
        ...
        # Export data
        export_result = await verification_service.export_verification_data(
            user_id=test_user.id, format_type="csv"
        )
    ...
    @pytest.mark.asyncio
    async def test_get_user_verification_not_found(
        self, verification_service, test_user
    ):
        ...
        with pytest.raises(ValueError, match="Verification .* not found"):
            await verification_service._get_user_verification(
                test_user.id, "invalid-id"
            )

    @pytest.mark.asyncio
    async def test_get_user_verification_wrong_user(
        self, verification_service, test_user, db_session
    ):
        ...
        verification = await verification_service.create_verification(
            other_user.id, "whatsapp"
        )

        # Try to access with test_user
        with pytest.raises(ValueError, match="Verification .* not found"):
            await verification_service._get_user_verification(
                test_user.id, verification.id
            )
