...
class TestSubscriptionService:
    ...
    async def test_get_user_subscription(self, subscription_service, mock_user):
        ...
        subscription_service.db.query.return_value.filter.return_value.first.return_value = (
            mock_user
        )

        # Mock usage calculation
        subscription_service._calculate_current_usage = AsyncMock(
            return_value={
                "sms_sent": 25,
                "voice_minutes": 10,
                "verifications": 5,
        ...
        subscription_service._calculate_subscription_costs = AsyncMock(
            return_value={
        ...
        result = await subscription_service.get_user_subscription(
    ...
    async def test_purchase_subscription_success(self, subscription_service, mock_user):
        ...
        subscription_service._process_payment = AsyncMock(
            return_value={
                "success": True,
        ...
        result = await subscription_service.purchase_subscription(
            user_id="test_user_id", plan_id="basic", billing_cycle="monthly"
        )
    ...
    async def test_purchase_subscription_invalid_plan(
        self, subscription_service, mock_user
    ):
        ...
        with pytest.raises(ValueError, match="Invalid subscription plan"):
            await subscription_service.purchase_subscription(
                user_id="test_user_id", plan_id="invalid_plan"
            )

    @pytest.mark.asyncio
    async def test_purchase_subscription_payment_failed(
        self, subscription_service, mock_user
    ):
        ...
        # Mock failed payment
        subscription_service._process_payment = AsyncMock(
            return_value={"success": False, "error": "Payment declined"}
        )

        with pytest.raises(Exception, match="Payment failed"):
            await subscription_service.purchase_subscription(
                user_id="test_user_id", plan_id="basic"
            )
    ...
    async def test_renew_subscription_success(self, subscription_service, mock_user):
        ...
        result = await subscription_service.renew_subscription(
            user_id="test_user_id", auto_renew=False
        )
    ...
    async def test_cancel_subscription_immediate(self, subscription_service, mock_user):
        ...
        result = await subscription_service.cancel_subscription(
            user_id="test_user_id", immediate=True, reason="User request"
        )
    ...
    async def test_cancel_subscription_end_of_period(
        ...
        result = await subscription_service.cancel_subscription(
            user_id="test_user_id", immediate=False, reason="Cost concerns"
        )
    ...
    async def test_track_usage_sms(self, subscription_service, mock_user):
        ...
        result = await subscription_service.track_usage(
            user_id="test_user_id", usage_type="sms_sent", amount=5
        )
    ...
    async def test_get_usage_analytics(self, subscription_service, mock_user):
        ...
        subscription_service.db.query.return_value.filter.return_value.count.return_value = (
            10
        )
    ...
    def test_check_usage_limits_within_limit(self, subscription_service):
        ...
        result = subscription_service._check_usage_limits(
            current_usage=50, limit=100, plan="basic"
        )
    ...
    def test_check_usage_limits_over_limit(self, subscription_service):
        ...
        result = subscription_service._check_usage_limits(
            current_usage=120, limit=100, plan="basic"
        )
    ...
    def test_check_usage_limits_unlimited(self, subscription_service):
        ...
        result = subscription_service._check_usage_limits(
            current_usage=1000, limit=-1, plan="enterprise"  # Unlimited
        )
    ...
    async def test_process_payment_free(self, subscription_service):
        ...
        result = await subscription_service._process_payment(
            user_id="test_user_id", amount=Decimal("0.00"), description="Free plan"
        )
    ...
    def test_generate_usage_recommendations_high_usage(self, subscription_service):
        ...
        recommendations = subscription_service._generate_usage_recommendations(
            current_plan="basic", utilization=utilization, total_cost=Decimal("15.00")
        )

        assert len(recommendations) > 0
        # Should recommend upgrade due to high SMS usage
        upgrade_rec = next(
            (rec for rec in recommendations if "upgrade" in rec.lower()), None
        )
    ...
    def test_generate_usage_recommendations_low_usage(self, subscription_service):
        ...
        recommendations = subscription_service._generate_usage_recommendations(
            current_plan="premium", utilization=utilization, total_cost=Decimal("5.00")
        )

        assert len(recommendations) > 0
        # Should recommend downgrade due to low usage
        downgrade_rec = next(
            (rec for rec in recommendations if "downgrade" in rec.lower()), None
        )
