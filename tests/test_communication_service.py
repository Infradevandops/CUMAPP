...
from services.communication_service import (
    CommunicationService, MessageType, CallStatus, create_communication_service
)
...
class TestCommunicationService:
    ...
    def communication_service(self, mock_db_session, mock_twilio_client):
        ...
        with patch('services.communication_service.SmartRoutingEngine'), \
             patch('services.communication_service.NotificationService'):
    ...
    @pytest.mark.asyncio
    async def test_send_sms_with_routing_success(self, communication_service, mock_user):
        """Test successful SMS sending with routing"""
        # Mock database queries
        communication_service.db.query.return_value.filter.return_value.first.return_value = mock_user
        
        # Mock phone numbers
        mock_phone_number = Mock()
        mock_phone_number.phone_number = "+1234567890"
        communication_service.db.query.return_value.filter.return_value.all.return_value = [mock_phone_number]
        ...
        communication_service.twilio_client.send_sms = AsyncMock(return_value=twilio_result)
        
        # Mock conversation creation
        mock_conversation = Mock()
        mock_conversation.id = "conversation_id"
        communication_service._get_or_create_conversation = AsyncMock(return_value=mock_conversation)
    ...
    async def test_send_sms_with_smart_routing(self, communication_service, mock_user):
        ...
        mock_phone_number = Mock()
        mock_phone_number.phone_number = "+1234567890"
        communication_service._get_user_phone_numbers = AsyncMock(return_value=[mock_phone_number])
    ...
    async def test_receive_sms_webhook(self, communication_service):
        ...
        communication_service.twilio_client.receive_sms_webhook = AsyncMock(return_value=processed_data)
        
        # Mock phone number lookup
        mock_phone_number = Mock()
        mock_phone_number.owner_id = "test_user_id"
        communication_service.db.query.return_value.filter.return_value.first.return_value = mock_phone_number
        ...
        communication_service.notification_service.send_verification_completed = AsyncMock()
        
        webhook_data = {"From": "+0987654321", "To": "+1234567890", "Body": "Hello world"}
    ...
    async def test_make_voice_call_with_routing(self, communication_service, mock_user):
        ...
        communication_service.twilio_client.make_call = AsyncMock(return_value=twilio_result)
    ...
    async def test_record_call(self, communication_service, mock_user):
        ...
        communication_service.db.query.return_value.filter.return_value.first.return_value = mock_call_message
        ...
        communication_service.twilio_client.record_call = AsyncMock(return_value=recording_result)
    ...
    async def test_forward_call(self, communication_service, mock_user):
        ...
        communication_service.twilio_client.forward_call = AsyncMock(return_value=forward_result)
    ...
    async def test_create_conference_call(self, communication_service, mock_user):
        ...
        conference_result = {
            "conference_sid": "CF123456",
            "status": "initiated"
        }
        communication_service.twilio_client.create_conference = AsyncMock(return_value=conference_result)
    ...
    @pytest.mark.asyncio
    async def test_get_user_number_dashboard(self, communication_service, mock_user, mock_phone_number):
        ...
        # Mock message counts
        communication_service.db.query.return_value.filter.return_value.count.return_value = 5
...
class TestCommunicationServiceFactory:
    ...
    def test_create_communication_service_success(self):
        ...
        with patch('services.communication_service.CommunicationService') as mock_service:
    ...
    def test_create_communication_service_exception(self):
        ...
        with patch('services.communication_service.CommunicationService', 
