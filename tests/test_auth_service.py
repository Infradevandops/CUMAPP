...
class TestAuthenticationService:
    ...
    async def test_user_registration_duplicate_email(self, db_session):
        ...
        await auth_service.register_user(
            email="test@example.com", username="testuser1", password="TestPassword123!"
        )
    ...
    async def test_user_login_success(self, db_session):
        ...
        session = (
            ...
            .filter(UserSession.refresh_token == result["refresh_token"])
            .first()
        )
    ...
    async def test_user_logout_success(self, db_session):
        ...
        session = (
            db_session.query(UserSession)
            .filter(UserSession.refresh_token == login_result["refresh_token"])
            .first()
        )
