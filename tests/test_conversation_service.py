...
class TestConversationCRUD:
    ...
    async def test_create_conversation_basic(self, conversation_service, test_users):
        ...
        conversation_data = ConversationCreate(
            title="Test Conversation", is_group=False, participant_ids=[participant.id]
        )
    ...
    async def test_create_sms_conversation(self, conversation_service, test_users):
        ...
        conversation_data = ConversationCreate(
            title="SMS Chat", is_group=False, external_number="+1234567890"
        )
    ...
    async def test_get_conversation_success(self, conversation_service, test_users):
        ...
        # Create conversation
        conversation_data = ConversationCreate(
            title="Test Get", participant_ids=[participant.id]
        )
    ...
    async def test_get_conversation_unauthorized(
        self, conversation_service, test_users
    ):
        ...
        # Create conversation
        conversation_data = ConversationCreate(
            title="Private Chat", participant_ids=[participant.id]
        )
    ...
    async def test_update_conversation(self, conversation_service, test_users):
        ...
        update_data = ConversationUpdate(
            title="Updated Title", is_archived=True, is_muted=True
        )
...
class TestParticipantManagement:
    ...
    async def test_add_participants(self, conversation_service, test_users):
        ...
        # Create conversation
        conversation_data = ConversationCreate(
            title="Add Participants Test", participant_ids=[initial_participant.id]
        )
...
class TestMessageOperations:
    ...
    async def test_send_chat_message(self, conversation_service, test_users):
        ...
        # Create conversation
        conversation_data = ConversationCreate(
            title="Message Test", participant_ids=[participant.id]
        )
        ...
        message_data = MessageCreate(
            content="Hello, world!", message_type=MessageType.CHAT
        )
    ...
    async def test_send_sms_message(self, conversation_service, test_users):
        ...
        conversation_data = ConversationCreate(
            title="SMS Test", external_number="+1234567890"
        )
    ...
    async def test_get_conversation_messages(self, conversation_service, test_users):
        ...
        # Create conversation
        conversation_data = ConversationCreate(
            title="Messages Test", participant_ids=[participant.id]
        )
    ...
    async def test_delete_message(self, conversation_service, test_users):
        ...
        # Delete message
        result = await conversation_service.delete_message(message.id, creator.id)
...
class TestSearchAndFiltering:
    ...
    async def test_get_user_conversations(self, conversation_service, test_users):
        ...
        for i in range(3):
            conversation_data = ConversationCreate(
                title=f"Conversation {i}", participant_ids=[participant.id]
            )
    ...
    @pytest.mark.asyncio
    async def test_get_user_conversations_with_filters(
        self, conversation_service, test_users
    ):
    ...
    async def test_mark_messages_as_read(self, conversation_service, test_users):
        ...
        # Create conversation
        conversation_data = ConversationCreate(
            title="Read Test", participant_ids=[participant.id]
        )
    ...
    async def test_get_unread_count(self, conversation_service, test_users):
        ...
        # Create conversation
        conversation_data = ConversationCreate(
            title="Unread Test", participant_ids=[participant.id]
        )
...
class TestErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_create_conversation_invalid_participants(
        self, conversation_service, test_users
    ):
