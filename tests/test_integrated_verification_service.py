...
from models.user_models import VerificationRequest
...
class TestIntegratedVerificationService:
    ...
    @pytest.fixture
    def integrated_service(self, mock_db_session, mock_textverified_client, mock_twilio_client):
        """Create Integrated Verification Service instance"""
        with patch('services.integrated_verification_service.VerificationService'), \
             patch('services.integrated_verification_service.SmartRoutingEngine'), \
             patch('services.integrated_verification_service.NotificationService'), \
             patch('services.integrated_verification_service.CodeExtractionService'):
    ...
    async def test_create_service_verification_success(self, integrated_service):
        ...
        integrated_service.verification_service.create_verification = AsyncMock(return_value=mock_verification)
        integrated_service.verification_service.get_verification_number = AsyncMock(return_value="+1234567890")
        ...
        if integrated_service.routing_engine:
            integrated_service.routing_engine.suggest_optimal_numbers = AsyncMock(return_value=mock_routing_recommendation)
    ...
    @pytest.mark.asyncio
    async def test_create_service_verification_unsupported_service(self, integrated_service):
    ...
    async def test_get_verification_status(self, integrated_service):
        ...
        integrated_service.verification_service._get_user_verification = AsyncMock(return_value=mock_verification)
        integrated_service.verification_service.check_verification_messages = AsyncMock(return_value=["Test message"])
        integrated_service.code_extractor.extract_verification_codes = Mock(return_value=["123456"])
    ...
    async def test_cancel_verification(self, integrated_service):
        ...
        integrated_service.verification_service.cancel_verification = AsyncMock(return_value=True)
        ...
        integrated_service.notification_service.send_verification_failed = AsyncMock(return_value=True)
    ...
    @pytest.mark.asyncio
    async def test_get_supported_services_with_category_filter(self, integrated_service):
    ...
    def test_calculate_duration_completed(self, integrated_service):
        ...
        mock_verification.completed_at = datetime(2024, 1, 1, 12, 1, 30)  # 90 seconds later
    ...
    async def test_generate_verification_analytics(self, integrated_service):
        ...
        mock_verification1.completed_at = datetime.utcnow() - timedelta(days=5) + timedelta(seconds=45)
        ...
        mock_verification3.completed_at = datetime.utcnow() - timedelta(days=1) + timedelta(seconds=30)
...
class TestIntegratedVerificationServiceFactory:
    ...
    def test_create_integrated_verification_service_success(self):
        ...
        with patch('services.integrated_verification_service.IntegratedVerificationService') as mock_service:
    ...
    def test_create_integrated_verification_service_exception(self):
        ...
        with patch('services.integrated_verification_service.IntegratedVerificationService', 
