...
class TestVerificationHistoryAPI:
    ...
    def test_get_verification_statistics(
        self, client, auth_headers, sample_verifications
    ):
    ...
    def test_get_verification_statistics_different_periods(
        self, client, auth_headers, sample_verifications
    ):
        """Test statistics with different time periods"""
        # Test 7-day period
        response = client.get(
            "/api/verifications/stats/summary?period_days=7", headers=auth_headers
        )
        ...
        response = client.get(
            "/api/verifications/stats/summary?period_days=1", headers=auth_headers
        )
...
class TestVerificationHistoryFiltering:
    ...
    def test_filter_by_date_range(self, client, auth_headers, sample_verifications):
        ...
        response = client.get(
            f"/api/verifications?date_from={date_from}", headers=auth_headers
        )
        ...
        for verification in data["verifications"]:
            created_date = datetime.fromisoformat(
                verification["created_at"].replace("Z", "")
            )
    ...
    def test_pagination_with_filters(self, client, auth_headers, sample_verifications):
        ...
        response = client.get(
            "/api/verifications?page=1&page_size=2", headers=auth_headers
        )
...
class TestVerificationHistoryAnalytics:
    ...
    def test_success_rate_calculation(self, client, auth_headers, sample_verifications):
        ...
        response = client.get("/api/verifications/stats/summary", headers=auth_headers)
    ...
    def test_status_breakdown_analytics(
        self, client, auth_headers, sample_verifications
    ):
        ...
        expected_statuses = {"completed": 2, "pending": 1, "failed": 1, "cancelled": 1}
...
class TestVerificationHistoryPerformance:
    ...
    def test_large_dataset_pagination(
        ...
        # Test first page
        response = client.get(
            "/api/verifications?page=1&page_size=20", headers=auth_headers
        )
        ...
        # Test middle page
        response = client.get(
            "/api/verifications?page=3&page_size=20", headers=auth_headers
        )
