...
class TestAIAssistantAPI:
    ...
    def test_create_conversation_success(
        self, mock_ai_service, sample_conversation_context
    ):
        """Test successful conversation creation"""
        mock_ai_service.create_conversation_context.return_value = (
    ...
    def test_add_message_to_conversation_success(
        self, mock_ai_service, sample_conversation_context
    ):
        ...
        with patch("api.ai_assistant_api.get_ai_service", return_value=mock_ai_service):
            response = client.post(
                "/api/ai/conversations/test_conv_1/messages",
                json={
                    "role": "user",
                    "content": "Can you help me with verification?",
    ...
    def test_add_message_empty_content(self):
        ...
        response = client.post(
            "/api/ai/conversations/test_conv_1/messages",
            json={"role": "user", "content": ""},  # Empty content
        )

        assert response.status_code == 422  # Validation error

    def test_get_conversation_messages_success(
        self, mock_ai_service, sample_conversation_context
    ):
        """Test successful retrieval of conversation messages"""
        mock_ai_service.conversation_contexts = {
            "test_conv_1": sample_conversation_context
        }
    ...
    def test_analyze_message_intent_success(self, mock_ai_service):
        ...
        mock_ai_service.analyze_message_intent.return_value = (
            IntentType.QUESTION,
            0.92,
        )
        ...
        with patch("api.ai_assistant_api.get_ai_service", return_value=mock_ai_service):
            response = client.post(
                ...
                json={"message": "How do I verify my WhatsApp account?"},
            )
    ...
    def test_analyze_message_intent_empty_message(self):
        ...
        response = client.post(
            "/api/ai/analyze-intent", json={"message": ""}  # Empty message
        )

        assert response.status_code == 422  # Validation error

    def test_generate_response_suggestions_success(
        self, mock_ai_service, sample_response_suggestion
    ):
    ...
    def test_provide_contextual_help_empty_query(self):
        ...
        response = client.post(
            "/api/ai/contextual-help", json={"query": ""}  # Empty query
        )
    ...
    def test_enhance_conversation_success(self, mock_ai_service):
        ...
        with patch("api.ai_assistant_api.get_ai_service", return_value=mock_ai_service):
            response = client.post(
                ...
                json={"conversation_id": "test_conv_1", "enhancement_type": "auto"},
            )
    ...
    def test_enhance_conversation_invalid_type(self):
        ...
        response = client.post(
            "/api/ai/enhance-conversation",
            json={"conversation_id": "test_conv_1", "enhancement_type": "invalid_type"},
        )
    ...
    def test_service_error_handling(self, mock_ai_service):
        ...
        with patch("api.ai_assistant_api.get_ai_service", return_value=mock_ai_service):
            response = client.post(
                "/api/ai/analyze-intent", json={"message": "Test message"}
            )
    ...
    def test_conversation_not_found_error(self, mock_ai_service):
        ...
        mock_ai_service.add_message_to_context.side_effect = ValueError(
            "Conversation context not found"
        )

        with patch("api.ai_assistant_api.get_ai_service", return_value=mock_ai_service):
            response = client.post(
                "/api/ai/conversations/nonexistent/messages",
...
class TestAIAssistantAPIIntegration:
    ...
    def test_full_conversation_workflow(self):
        ...
        response = client.post(
            "/api/ai/conversations/integration_test/messages",
            json={
                "role": "user",
        ...
        response = client.post(
            "/api/ai/analyze-intent",
            json={"message": "Can you help me verify my WhatsApp account?"},
        )
        assert response.status_code == 200
        ...
        response = client.post(
            "/api/ai/contextual-help",
            json={"query": "How do I verify my WhatsApp account?"},
        )
        assert response.status_code == 200
        ...
        response = client.post(
            "/api/ai/enhance-conversation",
            json={"conversation_id": "integration_test", "enhancement_type": "auto"},
        )
        assert response.status_code == 200
        ...
        assert any(
            conv["conversation_id"] == "integration_test" for conv in conversations
        )
