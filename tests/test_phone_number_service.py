...
class TestPhoneNumberService:
    ...
    async def test_search_available_numbers_us(self, phone_service):
        ...
        numbers, count = await phone_service.search_available_numbers(
            "US", "555", ["sms"], 5
        )
    ...
    async def test_purchase_phone_number_success(self, phone_service, mock_user):
        ...
        phone_service.db.query.return_value.filter.return_value.count.return_value = (
            0  # No existing numbers
        )

        result = await phone_service.purchase_phone_number(
            "test_user_1", "+15551234567"
        )
    ...
    async def test_purchase_phone_number_user_not_found(self, phone_service):
        ...
        phone_service.db.query.return_value.filter.return_value.first.return_value = (
            None
        )

        with pytest.raises(ValueError, match="User not found"):
            await phone_service.purchase_phone_number(
                "nonexistent_user", "+15551234567"
            )

    @pytest.mark.asyncio
    async def test_purchase_phone_number_already_owned(
        self, phone_service, mock_user, mock_phone_number
    ):
    ...
    async def test_get_owned_numbers(self, phone_service, mock_phone_number):
        ...
        phone_service.db.query.return_value.filter.return_value.filter.return_value.order_by.return_value.all.return_value = [
            mock_phone_number
        ]
        phone_service.db.query.return_value.filter.return_value.filter.return_value.order_by.return_value.count.return_value = (
            1
        )
    ...
    async def test_renew_phone_number_not_found(self, phone_service):
        ...
        with pytest.raises(
            ValueError, match="Phone number not found or not owned by user"
        ):
    ...
    @pytest.mark.asyncio
    async def test_cancel_phone_number_already_cancelled(
        self, phone_service, mock_phone_number
    ):
