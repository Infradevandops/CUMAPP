...
if not all([TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_PHONE_NUMBER]):
    logger.error("Twilio credentials missing. Please configure for production use.")
...
async def get_textverified_balance():
    ...
    if not textverified_client:
        raise HTTPException(
            status_code=503, detail="TextVerified service is not configured"
        )

    try:
        balance = await textverified_client.check_balance()
        return {"service": "TextVerified", "balance": balance, "currency": "USD"}
    except Exception as e:
...
async def get_textverified_services():
    ...
    try:
        ...
        return {"service": "TextVerified", "count": len(services), "services": services}
    except Exception as e:
...
async def send_sms(request: SMSRequest):
    ...
    if not twilio_client:
        raise HTTPException(
            status_code=503, detail="Twilio SMS service is not configured"
        )
...
async def create_verification(request: VerificationRequest):
    ...
    try:
        verification_id = await textverified_client.create_verification(
            service_name=request.service_name, capability=request.capability
        )
        ...
        logger.info(
            f"Created verification {verification_id} for service {request.service_name}"
        )
...
async def get_verification_number(verification_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get verification number: {str(e)}"
        )
...
async def get_verification_messages(verification_id: str):
    ...
    try:
        ...
        logger.info(
            f"Retrieved {len(messages)} messages for verification {verification_id}"
        )
        return {
            "verification_id": verification_id,
            "messages": messages,
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get verification messages: {str(e)}"
        )
    ...
    try:
        ...
        return {
            ...
            "message": (
                ...
                if success
                else "Failed to cancel verification"
            ),
        }
    except Exception as e:
...
## All mock Twilio endpoints removed. Only real Twilio client logic should remain.
...
async def suggest_response(request: AIRequest):
    ...
    try:
        suggestion = await groq_client.suggest_sms_response(
            conversation_history=request.conversation_history, context=request.context
        )

        return {"suggestion": suggestion, "model": GROQ_MODEL, "service": "Groq"}
    except Exception as e:
...
async def get_available_numbers(country_code: str):
    ...
    try:
        numbers = twilio_client.available_phone_numbers(country_code.upper()).list()
        return {
            "country_code": country_code.upper(),
            "available_numbers": [n.phone_number for n in numbers],
            "is_mock": False,
        }
    except Exception as e:
        logger.error(f"Failed to fetch available numbers: {e}")
        raise HTTPException(status_code=500, detail="Failed to fetch available numbers")
