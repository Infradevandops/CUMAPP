...
def upgrade():
    ...
    op.add_column('verification_requests', sa.Column('retention_until', sa.DateTime(), nullable=True))
    op.add_column('verification_requests', sa.Column('is_archived', sa.Boolean(), default=False))
    
    # Add retention fields to communication_messages
    op.add_column('communication_messages', sa.Column('retention_until', sa.DateTime(), nullable=True))
    op.add_column('communication_messages', sa.Column('is_archived', sa.Boolean(), default=False))
    
    # Add retention fields to voice_calls
    op.add_column('voice_calls', sa.Column('retention_until', sa.DateTime(), nullable=True))
    op.add_column('voice_calls', sa.Column('is_archived', sa.Boolean(), default=False))
    
    # Add retention fields to verification_messages
    op.add_column('verification_messages', sa.Column('retention_until', sa.DateTime(), nullable=True))
    op.add_column('verification_messages', sa.Column('is_archived', sa.Boolean(), default=False))
    
    # Add retention fields to payments
    op.add_column('payments', sa.Column('retention_until', sa.DateTime(), nullable=True))
    op.add_column('payments', sa.Column('is_archived', sa.Boolean(), default=False))
    
    # Add retention fields to usage_records
    op.add_column('usage_records', sa.Column('retention_until', sa.DateTime(), nullable=True))
    op.add_column('usage_records', sa.Column('is_archived', sa.Boolean(), default=False))
    ...
    op.create_index('idx_retention_table_active', 'data_retention_policies', ['table_name', 'is_active'], unique=False)
    ...
    op.create_index('idx_cleanup_status_created', 'cleanup_jobs', ['status', 'created_at'], unique=False)
    op.create_index('idx_cleanup_table_created', 'cleanup_jobs', ['table_name', 'created_at'], unique=False)
    ...
    op.create_index('idx_verification_retention', 'verification_requests', ['retention_until'], unique=False)
    op.create_index('idx_verification_archived', 'verification_requests', ['is_archived'], unique=False)
    
    op.create_index('idx_message_retention', 'communication_messages', ['retention_until'], unique=False)
    op.create_index('idx_message_archived', 'communication_messages', ['is_archived'], unique=False)
    
    op.create_index('idx_call_retention', 'voice_calls', ['retention_until'], unique=False)
    op.create_index('idx_call_archived', 'voice_calls', ['is_archived'], unique=False)
    
    op.create_index('idx_payment_retention', 'payments', ['retention_until'], unique=False)
    op.create_index('idx_payment_archived', 'payments', ['is_archived'], unique=False)
    
    op.create_index('idx_usage_retention', 'usage_records', ['retention_until'], unique=False)
    op.create_index('idx_usage_archived', 'usage_records', ['is_archived'], unique=False)
    ...
    try:
        ...
        tables_with_retention = [
            'verification_requests', 'communication_messages', 'voice_calls',
...
def downgrade():
    ...
    try:
        ...
        for table in tables_with_retention:
            op.execute(f"DROP TRIGGER IF EXISTS trigger_set_retention_date_{table} ON {table}")
        ...
        op.execute("DROP FUNCTION IF EXISTS calculate_retention_date(TIMESTAMP, INTEGER)")
