...
from core.circuit_breaker import CircuitBreaker, CircuitBreakerConfig, ServiceCircuitConfigs, circuit_manager
    ...
    async def report_error(
        ...
        if isinstance(error, BaseServiceException):
            if error.severity.value in ["high", "critical"]:
                logger.error(f"Service error in {service_name}.{operation}: {error}", exc_info=True)
            else:
                logger.warning(f"Service error in {service_name}.{operation}: {error}")
        else:
            logger.error(f"Unexpected error in {service_name}.{operation}: {error}", exc_info=True)
...
class ServiceErrorHandler:
    ...
    def _map_exception(self, error: Exception) -> Exception:
        ...
        if self.service_name.lower() == "textverified" and not isinstance(error, TextVerifiedException):
            # Try to map to TextVerified exception
            if hasattr(error, 'response') and hasattr(error.response, 'status_code'):
                return map_textverified_error(str(error.response.status_code), str(error))
        
        elif self.service_name.lower() == "twilio" and not isinstance(error, TwilioException):
...
class ErrorHandlerManager:
    ...
    def get_handler(self, service_name: str, config: Optional[ErrorHandlerConfig] = None) -> ServiceErrorHandler:
