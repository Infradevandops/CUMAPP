...
from models.user_models import User, Message, Conversation, PhoneNumber
...
class CommunicationService:
    ...
    async def send_sms_with_routing(self, user_id: str, to_number: str, message: str,
                                  from_number: str = None, use_smart_routing: bool = True,
                                  routing_options: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not from_number:
                user_numbers = await self._get_user_phone_numbers(user_id)
                from_number = user_numbers[0].phone_number if user_numbers else "+1555000001"
            ...
            logger.info(f"SMS sent successfully: {db_message.id} from {from_number} to {to_number}")
            return result
    ...
    async def receive_sms_webhook(self, webhook_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            phone_number_record = self.db.query(PhoneNumber).filter(
            ...
            await self.notification_service.send_verification_completed(
                user_id, "SMS", f"New message from {from_number}: {message_content[:50]}..."
            )
    ...
    async def make_voice_call_with_routing(self, user_id: str, to_number: str,
                                         from_number: str = None, twiml_url: str = None,
                                         use_smart_routing: bool = True,
                                         call_options: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(f"Voice call initiated: {db_message.id} from {from_number} to {to_number}")
            return result
    ...
    async def receive_voice_call_webhook(self, webhook_data: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    async def record_call(self, user_id: str, call_sid: str, 
                         recording_options: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            call_message = self.db.query(Message).filter(
                and_(
            ...
            recording_result = await self.twilio_client.record_call(
                call_sid=call_sid,
                **(recording_options or {})
            )
            
            # Update call message with recording info
            call_message.content += f" [Recording: {recording_result.get('recording_sid')}]"
            self.db.commit()
            ...
            logger.info(f"Call recording started: {recording_result.get('recording_sid')} for call {call_sid}")
            return result
    ...
    async def forward_call(self, user_id: str, call_sid: str, forward_to: str,
                          forwarding_options: Dict[str, Any] = None) -> Dict[str, Any]:
        """
    ...
    async def create_conference_call(self, user_id: str, conference_name: str,
                                   participants: List[str],
                                   conference_options: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(f"Conference call created: {conference_name} with {len(participants)} participants")
            return result
    ...
    async def get_conversation_history(self, user_id: str, conversation_id: str = None,
                                     external_number: str = None, limit: int = 50,
    ...
    async def get_call_history(self, user_id: str, limit: int = 50, 
                             offset: int = 0) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            calls = self.db.query(Message).join(Conversation).filter(
                and_(
                    Message.message_type == MessageType.VOICE.value,
                    or_(
                        Conversation.created_by == user_id,
            ...
            for call in calls:
                call_data = {
                    ...
                    "completed_at": call.delivered_at.isoformat() if call.delivered_at else None,
    ...
    async def get_user_number_dashboard(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for number in user_numbers:
                ...
                # Call statistics
                calls_made = self.db.query(Message).filter(
                    and_(
                        Message.from_number == number.phone_number,
                        Message.message_type == MessageType.VOICE.value,
                        Message.sender_id == user_id
                ...
                calls_received = self.db.query(Message).filter(
                    and_(
                        Message.to_number == number.phone_number,
                        Message.message_type == MessageType.VOICE.value,
                        Message.sender_id.is_(None)
                ...
                number_info = {
                    ...
                    "purchased_at": number.purchased_at.isoformat() if number.purchased_at else None,
                    ...
                    "costs": {
                        ...
                        "total_estimated_cost": estimated_sms_cost + estimated_call_cost
                    }
                }
            ...
            recent_messages = self.db.query(Message).join(Conversation).filter(
                and_(
            ...
            if self.routing_engine and user_numbers:
                try:
                    ...
                    recent_destinations = self.db.query(Message.to_number).filter(
                        and_(
                            Message.sender_id == user_id,
                            Message.created_at >= recent_date,
                    ...
                    routing_recommendations = analytics.get("optimization_opportunities", [])
                except Exception as e:
    ...
    async def manage_user_number(self, user_id: str, action: str, 
                               number_data: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    async def _get_or_create_conversation(self, user_id: str, 
                                        external_number: str) -> Conversation:
    ...
    async def _purchase_number_for_user(self, user_id: str, 
                                      number_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _release_user_number(self, user_id: str, 
                                 number_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get user's number
            db_number = self.db.query(PhoneNumber).filter(
                and_(
    ...
    async def _update_user_number(self, user_id: str, 
                                number_data: Dict[str, Any]) -> Dict[str, Any]:
...
# Factory function
def create_communication_service(db_session: Session, 
                               twilio_client: EnhancedTwilioClient = None) -> CommunicationService:
    """
