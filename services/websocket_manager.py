...
class AuthenticatedConnectionManager:
    ...
    async def broadcast_new_message(self, message: Message):
        ...
        sent_count = await self.send_to_conversation(
            message.conversation_id, message_data, exclude_user=message.sender_id
        )
    ...
    async def _get_conversation_participants(self, conversation_id: str) -> Set[str]:
        ...
        try:
            participants = (
                db.query(conversation_participants.c.user_id)
...
class AuthenticatedWebSocketHandler:
    ...
    async def handle_websocket(self, websocket: WebSocket, token: str):
        ...
        user_id = await self.connection_manager.authenticate_and_connect(
            websocket, token
        )
        if not user_id:
        ...
        try:
            ...
            await self.connection_manager.send_personal_message(
                user_id, {"type": "error", "message": "Invalid JSON format"}
            )
        except Exception as e:
    ...
    async def process_message(self, user_id: str, message: dict):
        ...
        try:
            if message_type == "ping":
                ...
                if message_id:
                    await self.connection_manager.handle_message_delivery(
                        user_id, message_id
                    )
                ...
                if conversation_id and content:
                    ...
                    try:
                        ...
                        message_data = MessageCreate(
                            content=content, message_type=MessageType(message_type_enum)
                        )
                ...
                conversation_id = message.get("conversation_id")
                if conversation_id:
                    success = await self.connection_manager.join_conversation(
                ...
                conversation_id = message.get("conversation_id")
                if conversation_id:
                    await self.connection_manager.leave_conversation(
                        user_id, conversation_id
                    )
                    await self.connection_manager.send_personal_message(
                        user_id,
            ...
            await self.connection_manager.send_personal_message(
                user_id, {"type": "error", "message": "Failed to process message"}
            )
