...
from models.user_models import User, PhoneNumber, Message, VerificationRequest
...
class SubscriptionService:
    ...
    async def get_subscription_plans(self, include_pricing: bool = True) -> Dict[str, Any]:
        """
            ...
            for plan_name, plan_config in self.subscription_plans.items():
                ...
                if include_pricing:
                    ...
                    plan_info["overage_rates"] = {
                        key: float(value) for key, value in plan_config["overage_rates"].items()
                    }
    ...
    async def get_user_subscription(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            plan_config = self.subscription_plans.get(current_plan, self.subscription_plans["free"])
            ...
            cost_breakdown = await self._calculate_subscription_costs(user_id, usage_stats)
            ...
            subscription_info = {
                ...
                "billing": {
                    ...
                    "next_billing_date": user.subscription_expires.isoformat() if user.subscription_expires else None,
    ...
    async def purchase_subscription(self, user_id: str, plan_id: str, 
                                  billing_cycle: str = "monthly",
                                  payment_method: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            limits = plan_config["limits"]
            user.monthly_sms_limit = limits["sms_monthly"] if limits["sms_monthly"] != -1 else 999999
            user.monthly_voice_minutes_limit = limits["voice_minutes_monthly"] if limits["voice_minutes_monthly"] != -1 else 999999
        ...
        try:
            ...
            if not payment_result["success"]:
                ...
                if auto_renew:
                    ...
                    raise Exception(f"Renewal payment failed: {payment_result['error']}")
    ...
    async def cancel_subscription(self, user_id: str, 
        ...
        try:
            ...
            if immediate:
                ...
                logger.info(f"User {user_id} subscription marked for cancellation on {cancellation_date}")
    ...
    async def track_usage(self, user_id: str, usage_type: str, 
        ...
        try:
            ...
            if usage_type == "sms_sent":
                ...
                user.monthly_voice_minutes_used = (user.monthly_voice_minutes_used or 0) + amount
                ...
                verification_count = self.db.query(VerificationRequest).filter(
                    and_(
            ...
            if limit_status["over_limit"]:
                ...
                if usage_type == "sms_sent":
                    ...
                    overage_cost = overage_amount * plan_config["overage_rates"]["voice_per_minute"]
                elif usage_type == "verifications":
                    overage_cost = overage_amount * plan_config["overage_rates"]["verification"]
    ...
    async def get_billing_history(self, user_id: str, limit: int = 50) -> Dict[str, Any]:
        """
    ...
    async def get_usage_analytics(self, user_id: str, period_days: int = 30) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Get SMS usage
            sms_sent = self.db.query(Message).filter(
                and_(
            ...
            sms_received = self.db.query(Message).filter(
                and_(
            ...
            voice_calls = self.db.query(Message).filter(
                and_(
                    Message.sender_id == user_id,
            ...
            verifications = self.db.query(VerificationRequest).filter(
                and_(
                    VerificationRequest.user_id == user_id,
            ...
            voice_cost = voice_calls * self.pricing_matrix["voice"]["domestic_per_minute"]  # Assuming 1 min per call
            verification_cost = verifications * self.pricing_matrix["verifications"]["standard"]
            phone_number_cost = phone_numbers * self.pricing_matrix["phone_numbers"]["monthly_rental"]
            
            total_usage_cost = sms_cost + voice_cost + verification_cost + phone_number_cost
            ...
            if limits["sms_monthly"] != -1:
                utilization["sms"] = (user.monthly_sms_used or 0) / limits["sms_monthly"] * 100
            else:
                utilization["sms"] = 0  # Unlimited
            
            if limits["voice_minutes_monthly"] != -1:
                utilization["voice"] = (user.monthly_voice_minutes_used or 0) / limits["voice_minutes_monthly"] * 100
            else:
                utilization["voice"] = 0  # Unlimited
            
            if limits["verifications_monthly"] != -1:
                current_month_verifications = self.db.query(VerificationRequest).filter(
                    and_(
                        VerificationRequest.user_id == user_id,
                        VerificationRequest.created_at >= datetime.utcnow().replace(day=1)
                    )
                ).count()
                utilization["verifications"] = current_month_verifications / limits["verifications_monthly"] * 100
            else:
    ...
    async def _calculate_subscription_costs(self, user_id: str, usage_stats: Dict[str, int]) -> Dict[str, Any]:
        ...
        # SMS overage
        if limits["sms_monthly"] != -1 and usage_stats["sms_sent"] > limits["sms_monthly"]:
        ...
        # Voice overage
        if limits["voice_minutes_monthly"] != -1 and usage_stats["voice_minutes"] > limits["voice_minutes_monthly"]:
            voice_overage = usage_stats["voice_minutes"] - limits["voice_minutes_monthly"]
            overage_costs["voice"] = float(voice_overage * overage_rates["voice_per_minute"])
            total_overage += voice_overage * overage_rates["voice_per_minute"]
        
        # Verification overage
        if limits["verifications_monthly"] != -1 and usage_stats["verifications"] > limits["verifications_monthly"]:
            verification_overage = usage_stats["verifications"] - limits["verifications_monthly"]
            overage_costs["verifications"] = float(verification_overage * overage_rates["verification"])
    ...
    def _check_usage_limits(self, current_usage: int, limit: int, plan: str) -> Dict[str, Any]:
    ...
    async def _process_payment(self, user_id: str, amount: Decimal, 
                             description: str, payment_method: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    def _generate_usage_recommendations(self, current_plan: str, 
                                      utilization: Dict[str, float],
                                      total_cost: Decimal) -> List[str]:
        ...
        if all(percentage < 30 for percentage in utilization.values()) and current_plan != "free":
            recommendations.append(
