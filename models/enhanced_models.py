...
from sqlalchemy import (
    ...
    Enum, Integer, Numeric, Index, JSON, UniqueConstraint
)
...
class UserNumber(Base):
    ...
    status = Column(String(20), default="active")  # active, suspended, expired, cancelled
    ...
    sent_messages = relationship("EnhancedMessage", foreign_keys="EnhancedMessage.from_number_id", back_populates="from_number_obj")
    received_messages = relationship("EnhancedMessage", foreign_keys="EnhancedMessage.to_number_id", back_populates="to_number_obj")
...
class EnhancedMessage(Base):
    ...
    from_number_obj = relationship("UserNumber", foreign_keys=[from_number_id])
...
class CountryRouting(Base):
    ...
    # Routing preferences
    preferred_routing_type = Column(Enum(RoutingType), default=RoutingType.SMART_ROUTING)
